...
const SignUp = () => {
 ...
 const handleSignUp = async (e) => {
    ...
    if (response.ok) {
        // 1. Save the user data to localStorage IMMEDIATELY
        // We use CleanName and CleanEmail straight from the form
        localStorage.setItem("user", JSON.stringify({ 
          name: CleanName, 
          email: CleanEmail 
        }));
        
        // 2. Alert and redirect to Home
        alert("Account created! Welcome to Voice Shield.");
        navigate("/"); 
        
        // 3. Force the page to refresh so the Navbar notices the new login
...
const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        setIsLoading(true); // Assuming you added this state!
        setErrorMsg("");
        ...
        const backendRes = await fetch("http://127.0.0.1:5000/google-login", {
          ...
          body: JSON.stringify({
            ...
            action: "signup" // <--- Tells Flask we are on the Signup page
          }),
        });
        ...
        if (backendRes.ok) {
          ...
          // THE 409 CATCHER: User already exists!
          setErrorMsg("Account already exists with this Google email. Please go to Sign In.");
        } else {
      ...
      <div className="card">
        ...
        <div className="social-container">
          ...
          <button className="btn-social" title="Google" type="button" onClick={() => googleLogin()}>
           ...
           {/* GitHub SVG */}
          <button className="btn-social" title="GitHub">
            <svg viewBox="0 0 24 24">
